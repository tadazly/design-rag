...
- GitHub Release 只包含两个 Codex Plugin 本地/离线安装 ZIP、Windows GUI EXE、macOS GUI DMG、`SHA256SUMS.txt` 和发布说明。发布说明必须用精确文件名解释每项用途，并显示 Windows/macOS 签名及 macOS notarization 状态；evidence 只进入 Actions 审计 artifact。
